...
def stamp_and_sign(input_folder):
    passcode = input("Enter the passcode to proceed: ")
    if passcode != '54231':
        print("Incorrect passcode. Access denied.")
        return

    stamped_signed_path = os.path.join(os.path.dirname(__file__), 'img', 'stamped_signed.png')
    if not os.path.exists(stamped_signed_path):
        print("stamped_signed.png not found in the img folder.")
        return

    # Resize the stamp image to 300x300 pixels
    stamp_image = Image.open(stamped_signed_path)
    stamp_image_resized = stamp_image.resize((300, 300))

    # Get dimensions of A4 image (assuming A4 size is 8.27 x 11.69 inches at 300 DPI)
    a4_width = 8.27 * 300
    a4_height = 11.69 * 300

    # Calculate position to place stamp image in bottom-right rectangle
    stamp_width, stamp_height = stamp_image_resized.size
    stamp_position = (int(a4_width - stamp_width), int(a4_height - stamp_height))
    ...
    for file in os.listdir(input_folder):
        if file.lower().endswith(('png', 'jpg', 'jpeg', 'bmp', 'gif')):
            ...
            # Paste resized stamp image onto A4 image
            image.paste(stamp_image_resized, stamp_position, stamp_image_resized)

            # Save modified image
            output_path = os.path.join(input_folder, f"stamped_{file}")
            image.save(output_path)
            image_files.append(output_path)

    print("Stamp and signature image added successfully!")
    return image_files
    ...
    while True:
        ...
        while True:
            ...
            if choice == '1':
                ...
                format_choice = input("Select format:\n"
                                      "1. Background (2560*1400)\n"
                                      "2. Hero (1280*720)\n"
                                      "3. Banner (1200*400)\n"
                                      "4. Blog (1200*800)\n"
                                      "5. Logo (400*100)\n"
                ...
                image_files = stamp_and_sign(input_folder)
                if image_files:
                    # Optionally, you can directly convert these images to a PDF or perform another action
                    images_to_pdf(input_folder)  # Or any other function you want to call with these images
            elif choice == '0':
